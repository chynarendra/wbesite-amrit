...
class DistrictRequest extends FormRequest
{
    ...
    public function rules()
    {
        if ($this->method() == 'PUT') {
            ...
            $rules = [
                'name_np' => 'required|min:3|max:200|unique:district,name_np,' . $post_id,
                'name_en' => 'required|min:3|max:200|unique:district,name_en,' . $post_id,
                'province_id' => 'required',
            ];
        } else {
            $rules = [
                'name_np' => 'required|min:3|max:200|unique:district,name_np',
                'name_en' => 'required|min:3|max:200|unique:district,name_en',
                'province_id' => 'required',
            ];
        }
        return $rules;
