...
class ProvinceRequest extends FormRequest
{
    ...
    public function rules()
    {
        if ($this->method() == 'PUT') {
            ...
            $rules = [
                'name_np' => 'required|min:3|max:200|unique:province,name_np,' . $post_id,
                'name_en' => 'required|min:3|max:200|unique:province,name_en,' . $post_id,
            ];
        } else {
            $rules = [
                'name_np' => 'required|min:3|max:200|unique:province,name_np',
                'name_en' => 'required|min:3|max:200|unique:province,name_en',
            ];
        }
        return $rules;
    }
