...
class SourceQueryRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        if ($this->method() == 'PUT') {
            ...
            $rules = [
                'name' => 'required|min:3|max:200|unique:source_query,name,' . $post_id
            ];
        } else {
            $rules = [
                'name' => 'required|min:3|max:200|unique:source_query,name'
            ];
        }
        return $rules;
    }
