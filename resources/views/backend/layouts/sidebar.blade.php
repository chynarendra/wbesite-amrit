...
<aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <a href="{{url('/dashboard')}}" class="brand-link">
        <img src="@if(systemSetting()->app_logo !=null){{asset('/storage/uploads/files/'.systemSetting()->app_logo)}} @else {{url('images/logo.png')}}   @endif"
        ...
        <div class="user-panel mt-3 pb-3 mb-3 d-flex">
            ...
            <div class="info">
                <a href="{{url('my-profile')}}"
                   class="d-block">@if(isset(Auth::user()->full_name)){{Auth::user()->full_name}}@endif</a>
            </div>
        </div>
                                            ...
                                            <li class="nav-item">
                                                <a href="{{url("$secondLevelMenu->menu_link")}}"
                                                   class="nav-link <?php activeTabHome($secondLevelMenu->menu_controller, $secondLevelMenu->parent_id);?>">
                                                    <i class="{!! $secondLevelMenu->menu_icon !!}"
                                                       aria-hidden="true"></i>
